...
async function draw() {
  ...
  dimensionsArea.boundedWidth =
    dimensionsArea.width -
    dimensionsArea.margin.left -
    dimensionsArea.margin.right;

  dimensionsArea.boundedHeight =
    dimensionsArea.height -
  ...
  const boundsArea = wrapperArea
    .append("g")
    .style(
      "translate",
      `transform(${dimensionsArea.margin.left}px, ${dimensionsArea.margin.top}px)`
    );

  //**************************3. Load Data and Create Accessors

  //----------------------Load Data
  const flow = await d3.csv("./flow.csv", (d) => {
    d3.autoType(d);
    return d;
  });
